...
# Legacy stage4 paths are read-only fallbacks for projects created before
# speaker script and lesson-plan outputs moved to stage3.
...
def project_deliverable_relpaths(
    ...
    *,
    ...
    return {
        ...
        "stage3_speaker_script": f"阶段3_逐字稿与教案输出/逐字稿/{topic}{SEPARATOR}逐字稿.md",
        "stage3_speaker_script_docx": f"阶段3_逐字稿与教案输出/逐字稿/{topic}{SEPARATOR}逐字稿.docx",
        "stage3_speaker_script_pdf": f"阶段3_逐字稿与教案输出/逐字稿/{topic}{SEPARATOR}逐字稿.pdf",
        "stage3_lesson_plan": f"阶段3_逐字稿与教案输出/教案设计/{topic}{SEPARATOR}教案设计.md",
        "stage3_lesson_plan_docx": f"阶段3_逐字稿与教案输出/教案设计/{topic}{SEPARATOR}教案设计.docx",
        "stage3_lesson_plan_pdf": f"阶段3_逐字稿与教案输出/教案设计/{topic}{SEPARATOR}教案设计.pdf",
    }
...
def existing_stage3_output_rel(
    run_dir: str | Path,
    artifact_key: str,
    *,
    state: dict[str, Any] | None = None,
    ...
    root = Path(run_dir)
    supported = {"stage3_speaker_script", "stage3_speaker_script_docx", "stage3_speaker_script_pdf"}
    if artifact_key not in supported:
        raise ValueError(f"unsupported stage3 artifact key: {artifact_key}")
    if manifest is None:
        manifest = _read_json(root / "_state" / "阶段3" / "speaker_script_manifest.json")
    relpaths = project_deliverable_relpaths(root, state=state, script=script)
    legacy = {
        "stage3_speaker_script": LEGACY_STAGE4_MARKDOWN_REL,
        "stage3_speaker_script_docx": LEGACY_STAGE4_DOCX_REL,
        "stage3_speaker_script_pdf": LEGACY_STAGE4_PDF_REL,
    }
    glob_dirs = {
        "stage3_speaker_script": [
            root / "阶段3_逐字稿与教案输出" / "逐字稿",
            root / "阶段3_逐字稿与教案输出",
        ],
        "stage3_speaker_script_docx": [
            root / "阶段3_逐字稿与教案输出" / "逐字稿",
            root / "阶段3_逐字稿与教案输出" / "docx",
        ],
        "stage3_speaker_script_pdf": [
            root / "阶段3_逐字稿与教案输出" / "逐字稿",
            root / "阶段3_逐字稿与教案输出" / "pdf",
        ],
    }
    suffixes = {
        "stage3_speaker_script": ".md",
        "stage3_speaker_script_docx": ".docx",
        "stage3_speaker_script_pdf": ".pdf",
    }
    return _first_existing_relpath(
        root,
        [
            ...
            legacy[artifact_key],
            *_glob_relpaths(root, glob_dirs[artifact_key], f"*{SEPARATOR}逐字稿{suffixes[artifact_key]}"),
        ],
    )


def existing_stage3_lesson_plan_output_rel(
    run_dir: str | Path,
    artifact_key: str,
    *,
    state: dict[str, Any] | None = None,
    ...
    root = Path(run_dir)
    supported = {"stage3_lesson_plan", "stage3_lesson_plan_docx", "stage3_lesson_plan_pdf"}
    if artifact_key not in supported:
        raise ValueError(f"unsupported stage3 lesson plan artifact key: {artifact_key}")
    if manifest is None:
        manifest = _read_json(root / "_state" / "阶段3" / "lesson_plan_manifest.json")
    relpaths = project_deliverable_relpaths(root, state=state, script=lesson_plan)
    legacy = {
        "stage3_lesson_plan": LEGACY_STAGE4_LESSON_PLAN_MARKDOWN_REL,
        "stage3_lesson_plan_docx": LEGACY_STAGE4_LESSON_PLAN_DOCX_REL,
        "stage3_lesson_plan_pdf": LEGACY_STAGE4_LESSON_PLAN_PDF_REL,
    }
    glob_dirs = {
        "stage3_lesson_plan": [root / "阶段3_逐字稿与教案输出" / "教案设计"],
        "stage3_lesson_plan_docx": [
            root / "阶段3_逐字稿与教案输出" / "教案设计",
            root / "阶段3_逐字稿与教案输出" / "教案设计" / "docx",
        ],
        "stage3_lesson_plan_pdf": [
            root / "阶段3_逐字稿与教案输出" / "教案设计",
            root / "阶段3_逐字稿与教案输出" / "教案设计" / "pdf",
        ],
    }
    suffixes = {
        "stage3_lesson_plan": ".md",
    ...
    return _first_existing_relpath(
        ...
        [
            ...
            relpaths[artifact_key],
            legacy[artifact_key],
            *_glob_relpaths(root, glob_dirs[artifact_key], f"*{SEPARATOR}教案设计{suffixes[artifact_key]}"),
        ],
    )
...
def _glob_relpaths(root: Path, directories: list[Path], pattern: str) -> list[str]:
    relpaths: list[str] = []
    seen: set[str] = set()
