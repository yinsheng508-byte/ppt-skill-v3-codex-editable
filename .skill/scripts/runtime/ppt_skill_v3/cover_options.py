...
DEFAULT_IMAGE_API_ENDPOINT = DEFAULT_IMAGE_API_GENERATION_ENDPOINT
...
def dispatch_cover_option_packets(
    ...
    *,
    ...
    from .image_prompt_plan import current_plan, compile_current_prompt
    # Candidate cards carry the resolved plan. Preserve authored attributes;
    # never replace palette, medium and type with three copies of common prose.
    for option_id, card in style_cards.items():
        plan, _ = current_plan(root, cover_slide["slide_index"], option_id=option_id)
        card["image_prompt_plan"] = plan
        card["composition"] = "；".join(plan["visual"])
        card["prompt_constraints"] = plan["constraints"]
    ...
    for packet_index, option_id in enumerate(COVER_OPTION_IDS, start=1):
        ...
        prompt = compile_current_prompt(root, cover_slide["slide_index"], option_id=option_id)
...
def cover_option_results_complete(run_dir: str | Path) -> bool:
    ...
    for option_id in COVER_OPTION_IDS:
        path = root / "_state" / "阶段2" / "cover_options" / "results" / f"cover_option_{option_id}.json"
        if not path.exists():
            return False
        try:
            ...
            return False
        if result.get("purpose") != "cover_option" or result.get("option_id") != option_id or not result_is_current(root, result):
            return False
    return True
...
def promote_selected_cover_option(run_dir: str | Path, *, force: bool = False) -> Path:
    ...
    if not result_is_current(root, cover_result):
        raise ValidationError("已选封面图片或风格依据已变化，请主控复核接纳或重新生成")
    ...
    if formal_result_path.exists() and not force:
        ...
        if existing.get("promoted_from_cover_option") and existing.get("selected_cover_option") == option_id and existing.get("image_sha256") == cover_result.get("image_sha256") and result_is_current(root, existing):
...
def _next_action_for_route(route: str, description: str) -> str:
    normalized = normalize_image_generation_route(route)
    if normalized == IMAGE_ROUTE_OPENAI_IMAGE_API:
        return f"主控大模型必须运行图片 API 批量执行器，按 6 并发生成{description}，优先目标 {DEFAULT_PPT_IMAGE_SIZE_16_9}，允许生成器返回存在轻微尺寸差异；尽量保留生成原始尺寸，并逐张记录 API evidence、尺寸和图片 sha"
    if normalized == IMAGE_ROUTE_CODEX_IMAGE_GEN:
        return f"主控大模型必须调用 Codex 内置 image_gen 工具生成{description}，优先目标 {DEFAULT_PPT_IMAGE_SIZE_16_9}，允许生成器返回存在轻微尺寸差异；尽量保留生成原始尺寸，把输出复制到项目目录，并逐张记录 image_gen evidence、尺寸和图片 sha"
    raise ValidationError(f"unsupported image generation route: {route}")
...
def _style_cards_from_design_contract(
    ...
    for option_id in COVER_OPTION_IDS:
        ...
        cards[option_id] = {
            ...
            "visual_medium": _field_or_default(option, "visual_medium", "使用当前图片风格确定的质感"),
...
def _build_cover_prompt(
    ...
    return render_image_prompt(cover_slide.get("final_visible_text"), style_card.get("image_prompt_plan"), {}, cover_slide.get("slide_index", 1), cover=True)
