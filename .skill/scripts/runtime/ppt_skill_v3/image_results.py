...
from .image_geometry import ASPECT_RATIO_RESULT_FIELDS, read_image_dimensions, review_image_aspect_ratio
from .image_routes import (
    CODEX_IMAGE_GEN_TOOL,
    DEFAULT_PPT_IMAGE_HEIGHT_TOLERANCE_PX,
    DEFAULT_PPT_IMAGE_SIZE_16_9,
    DEFAULT_PPT_IMAGE_WIDTH_TOLERANCE_PX,
...
def record_image_result(
    ...
    *,
    ...
    if stage != "stage2":
        raise ValidationError("record-image-result only supports formal stage2 image packets")
    ...
    target_tolerance_contract = _target_size_tolerance_contract(packet)
    ...
    if resolved_canonical_policy == "normalize_at_stage2":
        ...
        canonical_dir = root / "_state" / "阶段2" / canonical_dirname
        ...
        postprocess_manifest_path = root / "_state" / "阶段2" / "postprocess" / f"{packet.get('packet_id', Path(image_path).stem)}.json"
    ...
    size_review = _size_review(
        dimensions,
        target_size_contract,
        preferred_size_contract,
        target_tolerance_contract,
        extra_warnings=postprocess_size_warnings,
    )
    image_sha256 = _sha256_file(recorded_image_path)
    resolved_response_format = _image_api_response_format(packet, response_format)
    ...
    result = {
        ...
        "target_size_tolerance_px": {
            "width_px": target_tolerance_contract[0],
            "height_px": target_tolerance_contract[1],
        },
        "size_policy": size_policy,
        "postprocess_policy": postprocess_policy,
        "meets_preferred_size": size_review["meets_preferred_size"],
        "meets_target_size": size_review["meets_target_size"],
        **{field: size_review[field] for field in ASPECT_RATIO_RESULT_FIELDS},
    ...
    for field in ("generation_request_id", "image_style_hash", "image_style_snapshot", "generation_basis"):
    ...
    stale = not packet_matches_current(root, packet)
    if stale:
        request_key = packet.get("generation_request_id") or hashlib.sha256((str(generation_id) + image_sha256).encode()).hexdigest()
    ...
    if stale:
        append_event(root, "old_style_image_recorded_as_history", "runtime", slide_index=slide_index)
        return result

    state = read_state(root)
    state["required_actor"] = "main_controller"
    state["user_artifacts"]["stage2_image_prompts"] = PROMPTS_RELPATH
    ...
    if document_error:
        state["required_actor"] = "main_controller"
        state["next_required_action"] = "图片已成功保存，提示词文档待修复；先重建文档，不要重复生图：" + document_error
...
def _stage2_formal_results_complete(root: Path) -> bool:
    try:
    ...
    for slide in plan["slides"]:
        ...
        try:
            ...
            return False
        if result.get("stage") != "stage2" or result.get("purpose") != "full_slide" or not result_is_current(root, result):
            return False
...
def _visible_image_relpath(stage: str, slide_index: int, suffix: str) -> str:
    return _visible_image_relpath_for_purpose(stage, "full_slide", slide_index, None, suffix)
...
def _visible_image_relpath_for_purpose(stage: str, purpose: str, slide_index: int, option_id: object, suffix: str) -> str:
    ...
    if stage == "stage2":
        return f"阶段2_图片版PPT/img/slide_{slide_index:03d}{suffix}"
    raise ValidationError("stage3 background image output was removed from the formal workflow")
...
def _target_size_tolerance_contract(packet: dict[str, Any]) -> tuple[int, int]:
    for source in _size_contract_sources(packet):
        value = source.get("target_size_tolerance_px")
        if isinstance(value, dict):
            width = value.get("width_px")
            height = value.get("height_px")
            if (
                ...
                return width, height
    return DEFAULT_PPT_IMAGE_WIDTH_TOLERANCE_PX, DEFAULT_PPT_IMAGE_HEIGHT_TOLERANCE_PX
...
def _size_review(
    ...
    target_tolerance: tuple[int, int],
    *,
    ...
    meets_target = abs(width - target_size[0]) <= target_tolerance[0] and abs(height - target_size[1]) <= target_tolerance[1]
    aspect_review = review_image_aspect_ratio(width, height)
    if not meets_preferred:
        warnings.append(f"below_preferred_1k_size_{preferred_size[0]}x{preferred_size[1]}")
    if not aspect_review["meets_target_aspect_ratio"]:
        warnings.append("outside_target_aspect_ratio_16_9_tolerance_2_percent")
    return {
...
def _result_path(root: Path, stage: str, purpose: str, slide_index: int, option_id: object) -> Path:
    ...
    raise ValidationError("stage3 background image results were removed from the formal workflow")
...
def _image_api_response_format(packet: dict[str, Any], response_format: str | None) -> str:
    ...
    if isinstance(image_input, dict):
        packet_response_format = image_input.get("response_format")
        if isinstance(packet_response_format, str) and packet_response_format.strip():
            return packet_response_format.strip()
    return DEFAULT_IMAGE_API_RESPONSE_FORMAT
...
def _validate_api_evidence(root: Path, packet: dict[str, Any], result: dict[str, Any]) -> None:
    ...
    if result["api_endpoint"] == "/v1/images/generations" or evidence.get("response_format") is not None:
        expected["response_format"] = result["response_format"]
