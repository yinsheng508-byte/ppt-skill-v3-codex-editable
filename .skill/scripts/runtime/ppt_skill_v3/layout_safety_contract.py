...
def validate_layout_safety_contract(data: Any) -> dict[str, Any]:
    ...
    for position, slide in enumerate(slides, start=1):
        ...
        text_regions = slide.get("text_safe_regions", slide.get("editable_text_regions"))
        text_regions_label = "text_safe_regions" if "text_safe_regions" in slide else "editable_text_regions"
        _validate_regions(text_regions, f"{label}.{text_regions_label}", required=True)
        ...
        text_page_layer = policy.get("text_page_layer", policy.get("editable_page_layer"))
        for field, value in (("text_page_layer", text_page_layer), ("raster_visual_layer", policy.get("raster_visual_layer"))):
