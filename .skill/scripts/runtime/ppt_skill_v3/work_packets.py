...
def create_work_packet(
    ...
    *,
    ...
    packet = {
        ...
        "input_hashes": _input_hashes(root, artifacts),
...
def _input_artifacts_for_stage(stage: str, artifacts: list[str]) -> list[str]:
    values = list(dict.fromkeys(artifacts))
    if stage == "stage2":
        for item in (
            "阶段1_规划确认/PPT一致性.md",
            "阶段1_规划确认/图片风格.md",
...
def _input_hashes(root: Path, artifacts: list[str]) -> dict[str, str]:
    hashes: dict[str, str] = {}
    for relpath in artifacts:
        path = root / relpath
        if path.is_file():
            hashes[relpath] = f"sha256:{hashlib.sha256(path.read_bytes()).hexdigest()}"
        elif path.exists():
            hashes[relpath] = "present-directory"
        else:
