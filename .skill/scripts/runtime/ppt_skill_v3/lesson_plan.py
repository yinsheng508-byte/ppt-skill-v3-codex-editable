...
def build_lesson_plan(run_dir: str | Path, lesson_plan_json: str | Path) -> dict[str, Any]:
    ...
    if state["current_stage"] != "stage3" or state["status"] not in {
        ...
        raise ValidationError("lesson plan generation requires stage3 ready_for_stage3_script state")
    if not stage3_lesson_plan_required(state):
        raise ValidationError("lesson plan generation requires K12 stage3_outputs.lesson_plan.required=true")
    ...
    markdown_rel = relpaths["stage3_lesson_plan"]
    docx_rel = relpaths["stage3_lesson_plan_docx"]
    pdf_rel = relpaths["stage3_lesson_plan_pdf"]
    ...
    lesson_plan_output = stage3_outputs.setdefault("lesson_plan", {})
    lesson_plan_output["required"] = True
    lesson_plan_output["status"] = "generated"
    state.setdefault("user_artifacts", {})["stage3_lesson_plan"] = markdown_rel
    state["user_artifacts"]["stage3_lesson_plan_docx"] = docx_rel
    state["user_artifacts"]["stage3_lesson_plan_pdf"] = pdf_rel
    state.setdefault("expected_user_paths", {})["stage3_lesson_plan"] = markdown_rel
    state["expected_user_paths"]["stage3_lesson_plan_docx"] = docx_rel
    state["expected_user_paths"]["stage3_lesson_plan_pdf"] = pdf_rel
    state.setdefault("quality", {})["stage3"] = "pending_controller_review"
    state["next_required_action"] = "主控大模型检查阶段3逐字稿与教案全部产物；通过后记录阶段3输出完成决策并进入阶段4整理"
