...
def stage4_education_context_path(run_dir: str | Path) -> Path:
    return state_dir(run_dir) / "阶段4" / "education_context.json"


def stage4_textbook_context_path(run_dir: str | Path) -> Path:
    return state_dir(run_dir) / "阶段4" / "textbook_context.json"


def lesson_plan_context_path(run_dir: str | Path) -> Path:
    return state_dir(run_dir) / "阶段4" / "lesson_plan_context.json"
...
def build_lesson_plan_context(run_dir: str | Path) -> dict[str, Any]:
    ...
    context = {
        ...
        "locked_presentation_source": state.get("stage4_locked_presentation_source"),
    ...
    write_json(stage4_education_context_path(root), education_context)
    write_json(stage4_textbook_context_path(root), {"schema_version": SCHEMA_VERSION, "materials": textbook_context})
...
def _missing_core_fields(education_context: dict[str, Any], root: Path, state: dict[str, Any]) -> list[str]:
    ...
    if not state.get("stage4_locked_presentation_source"):
        missing.append("stage4_locked_presentation_source")
    return missing
