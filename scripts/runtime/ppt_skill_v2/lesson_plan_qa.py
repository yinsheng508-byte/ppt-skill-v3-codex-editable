...
from .deliverable_naming import existing_stage4_lesson_plan_output_rel
...
QA_REL = "_state/阶段4/lesson_plan_qa.json"
MANIFEST_REL = "_state/阶段4/lesson_plan_manifest.json"
...
def record_lesson_plan_qa(run_dir: str | Path, review_file: str | Path) -> Path:
    ...
    if state.get("current_stage") != "stage4":
        raise ValidationError("lesson plan QA can only be recorded in stage4")
    if not stage4_lesson_plan_required(state):
        raise ValidationError("lesson plan QA requires K12 stage4_outputs.lesson_plan.required=true")
    ...
    state.setdefault("runtime_artifacts", {})["stage4_lesson_plan_qa"] = QA_REL
    state.setdefault("quality", {})["stage4"] = "pending_controller_review"
    state["required_actor"] = "main_controller"
    state["next_required_action"] = "主控大模型检查阶段4讲稿与教案全部产物；通过后记录阶段4输出完成决策"
...
def _require_passable_lesson_plan_outputs(root: Path, state: dict[str, Any]) -> dict[str, Any]:
    ...
    for artifact_key, label in [
        ("stage4_lesson_plan", "Markdown"),
        ("stage4_lesson_plan_docx", "DOCX"),
        ("stage4_lesson_plan_pdf", "PDF"),
    ]:
        relpath = existing_stage4_lesson_plan_output_rel(root, artifact_key, state=state, manifest=manifest)
        if not relpath:
    ...
    for expected in [
        existing_stage4_lesson_plan_output_rel(root, "stage4_lesson_plan", state=state, manifest=manifest),
        existing_stage4_lesson_plan_output_rel(root, "stage4_lesson_plan_docx", state=state, manifest=manifest),
        existing_stage4_lesson_plan_output_rel(root, "stage4_lesson_plan_pdf", state=state, manifest=manifest),
    ]:
