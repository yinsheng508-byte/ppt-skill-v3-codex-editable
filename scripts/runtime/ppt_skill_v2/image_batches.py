...
def create_image_generation_batch(
    ...
    *,
    ...
    manifest = {
        ...
        "packet_paths": [_relative_or_text(root, path) for path in packet_paths],
        "completed_packets": 0,
...
def require_image_generation_batch_for_packet(run_dir: str | Path, packet: dict[str, Any]) -> Path:
    ...
    if stage not in {"stage2", "stage3"}:
        raise ValidationError("image generation packet.stage must be stage2 or stage3")
    purpose = packet.get("purpose") or ("full_slide" if stage == "stage2" else "stage3_background")
    batch_path = image_generation_batch_path(root, stage, purpose, group["group_id"], route=route)
