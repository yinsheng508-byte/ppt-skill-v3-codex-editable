...
from .text_unit_split_plan import load_text_unit_split_plan, require_split_plan_text_matches_content, text_unit_split_plan_path
...
def create_text_ownership_map_draft(run_dir: str | Path, output_file: str | Path | None = None) -> Path:
    root = Path(run_dir)
    state = read_state(root)
    if state["current_stage"] != "stage3":
        raise ValidationError("text ownership map draft can only be created in stage3")
    content = apply_stage3_scope(validate_content_asset(load_content(root)), state, "content")
    split_plan = load_text_unit_split_plan(root) if text_unit_split_plan_path(root).exists() else None
    if split_plan is not None:
        split_plan = apply_stage3_scope(split_plan, state, "text_unit_split_plan", strict=True)
        require_matching_slide_indices(("content", content), ("text_unit_split_plan", split_plan))
        require_split_plan_text_matches_content(split_plan, content)
        slides = [_ownership_slide_from_split(split_slide) for split_slide in split_plan["slides"]]
        source = "text_unit_split_plan"
    else:
        slides = [_ownership_slide_from_content(content_slide) for content_slide in content["slides"]]
    ...
    draft = {
        ...
        "basis": {
            ...
            "notes": "Draft generated by runtime; controller must review editable/background ownership before formal background dispatch.",
        },
        "slides": slides,
    }
    validate_text_ownership_map(draft)
    require_ownership_text_matches_content(draft, content)
    if split_plan is not None:
        require_ownership_matches_split_plan(draft, split_plan)
    target = Path(output_file) if output_file else root / "_state" / "阶段3" / "drafts" / "text_ownership_map.draft.json"
...
def _ownership_slide_from_split(split_slide: dict[str, Any]) -> dict[str, Any]:
    slide_index = split_slide["slide_index"]
    editable_layer = []
    raster_layer = []
    for index, unit in enumerate(split_slide.get("units", []), start=1):
        ownership_id = f"s{slide_index:03d}_own_{index:03d}"
        if unit.get("restore_as_editable") is True:
            editable_layer.append(
                {
                    "ownership_id": ownership_id,
                    "split_unit_id": unit["split_unit_id"],
                    "text_source": unit["content_source"],
                    "expected_text": unit["text"],
                    "semantic_role": unit["semantic_role"],
                    "style_role": unit.get("style_role", unit["semantic_role"]),
                    "visual_group_id": unit.get("visual_group_id", f"s{slide_index:03d}_group_{index:03d}"),
                    "remove_from_background": unit.get("background_action") == "remove_and_restore",
            ...
            raster_layer.append(
                {
                    ...
                    "semantic_role": unit.get("semantic_role", "raster_visual_text"),
                    "preserve_in_background": True,
                    "expected_text": unit["text"],
                    "source_field": "text_unit_split_plan",
                }
            )
    return {
        "slide_index": slide_index,
        "editable_page_layer": editable_layer,
        "raster_visual_layer": raster_layer,
        "unresolved_items": [],
    }


def _ownership_slide_from_content(content_slide: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, text in enumerate(content_slide.get("final_visible_text", []), start=1):
        ...
        editable_layer.append(
            {
                "ownership_id": f"s{slide_index:03d}_own_{index:03d}",
                "text_source": "_state/阶段1/content.json#slides[].final_visible_text",
                "expected_text": text,
                "semantic_role": semantic_role,
                "style_role": semantic_role,
                "visual_group_id": f"s{slide_index:03d}_group_{index:03d}",
                "remove_from_background": True,
                "restore_as_editable": True,
            }
        )
    return {
