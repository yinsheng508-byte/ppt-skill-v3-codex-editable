...
DEFAULT_IMAGE_GENERATION_ROUTE = IMAGE_ROUTE_CODEX_IMAGE_GEN
DEFAULT_PPT_IMAGE_SIZE_16_9 = "2048x1152"
DEFAULT_PPT_IMAGE_WIDTH_PX = 2048
DEFAULT_PPT_IMAGE_HEIGHT_PX = 1152
PREFERRED_PPT_IMAGE_WIDTH_PX = 2048
PREFERRED_PPT_IMAGE_HEIGHT_PX = 1152
...
def batch_path_for_route(run_dir: str | Path, stage: str, purpose: str, group_id: str, route: str) -> Path:
    ...
    if stage_name == "stage3":
        return root / "_state" / "阶段3" / batch_dir_name_for_route(normalized) / f"{group_id}.json"
...
def evidence_path_for_route(run_dir: str | Path, packet: dict[str, Any], route: str) -> Path:
    ...
    if packet.get("stage") == "stage3":
        return root / "_state" / "阶段3" / evidence_dir / f"{packet_id}.json"
...
def output_path_for_route(run_dir: str | Path, packet: dict[str, Any], output_format: str, route: str) -> Path:
    ...
    if packet.get("stage") == "stage3":
        return root / "_state" / "阶段3" / output_dir / f"{packet_id}{suffix}"
    raise ValidationError("packet.stage must be stage2 or stage3")
