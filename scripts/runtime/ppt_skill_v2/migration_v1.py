...
def render_v1_summary_markdown(summary: dict[str, Any]) -> str:
    ...
    lines.extend(["", "> 这是只读摘要，不写入 v2 state，不接管 v1 runner。", ""])
    return "\n".join(lines)
...
def _infer_gaps(root: Path, agent_state: dict[str, Any], artifacts: list[str], state_files: list[str]) -> list[str]:
    ...
    if not (root / "controller_decisions").exists() and not (root / "_decisions").exists():
        gaps.append("未找到 v2 风格显式主控决策，阶段确认需要人工复核")
...
def _suggest_next_steps(likely_stage: str, gaps: list[str]) -> list[str]:
    ...
    if likely_stage == "阶段0_资料整理":
        steps.append("按 v2 新项目方式重新整理资料，并由主控大模型撰写阶段1规划。")
    elif likely_stage == "阶段1_规划确认":
        steps.append("把 v1 planning 内容作为参考，重新生成 v2 阶段1用户确认文档。")
    elif likely_stage == "阶段2_图片版PPT":
        steps.append("核对图片来源证据；缺少 provider 证据时按 v2 阶段2重新登记或返工。")
    else:
        steps.append("核对可编辑 PPTX 是否真实可打开、页数是否匹配，再决定是否进入 v2 阶段4演讲稿输出。")
    if gaps:
        steps.append("先解决缺口清单，再考虑创建 v2 项目承接。")
    return steps
