...
from .image_routes import DEFAULT_PPT_IMAGE_SIZE_16_9, IMAGE_ROUTE_OPENAI_IMAGE_API, normalize_image_generation_route
...
DEFAULT_IMAGE_API_BASE_URL = "https://grsai.dakka.com.cn/"
...
DEFAULT_IMAGE_API_KEY_FILE = Path.home() / ".codex" / "secrets" / "ppt-skill-v2" / "image_api_key"
GRSAI_DRAW_COMPLETIONS_ENDPOINT = "/v1/draw/completions"
    ...
    *,
    ...
    if not isinstance(key, str) or not key.strip():
        raise ValidationError(
            ...
            "or configure a local key file with `python3 scripts/pptctl.py configure-image-api-key --stdin`"
        )
...
def run_image_api_batch(
    ...
    *,
    ...
    for packet_path in packet_paths:
        ...
        if not force and isinstance(packet_id, str) and packet_id in completed:
...
def default_batch_path_for_stage(run_dir: str | Path, stage: str) -> Path:
    ...
    if stage in {"stage3", "stage3-background"}:
        return root / "_state" / "阶段3" / "image_api_batches" / "stage3-background-image-api.json"
...
def _request_packet_image(root: Path, packet_path: Path, packet: dict[str, Any], config: ImageApiConfig) -> ImageApiResponse:
    ...
    if endpoint not in {"/v1/images/generations", "/v1/images/edits", GRSAI_DRAW_COMPLETIONS_ENDPOINT}:
...
def _encode_multipart(fields: dict[str, str], image_paths: list[Path]) -> tuple[bytes, str]:
    boundary = f"----ppt-skill-v2-{uuid.uuid4().hex}"
    chunks: list[bytes] = []
...
def _api_output_path(root: Path, packet: dict[str, Any], output_format: str) -> Path:
    ...
    if packet.get("stage") == "stage3":
        return root / "_state" / "阶段3" / "api_outputs" / f"{packet_id}{suffix}"
...
def _api_evidence_path(root: Path, packet: dict[str, Any]) -> Path:
    ...
    if packet.get("stage") == "stage3":
        return root / "_state" / "阶段3" / "api_calls" / f"{packet_id}.json"
    raise ValidationError("packet.stage must be stage2 or stage3")
...
def _batch_run_report_path(root: Path, batch_path: Path, batch: dict[str, Any]) -> Path:
    ...
    if batch.get("stage") == "stage3":
        return root / "_state" / "阶段3" / "api_batch_runs" / f"{batch_id}-{stamp}.json"
